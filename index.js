...
function capitalizeNames() {
  ...
  const capital = items.slice();
  return capital.map((element) => element.name.toUpperCase());

  /* This can really be done more efficiently, but I needed to use slice for credit.
  return capital.map((element) => element.name.toUpperCase()); accomplishes with one line. */
}
...
function calculateTotalInventory() {
  ...
  return items.reduce((accumulator, currentvalue) =>  accumulator = accumulator + currentvalue.inventory, 0);
}
...
function calculateAllInventoryPrice() {
  ...
  return items.reduce((accumulator, currentvalue) =>  accumulator = accumulator + currentvalue.price, 0);
}
...
function getItemPriceByName(name) {
  ...
  for(const x in items){
    if(items.filter((item) => item.name === name)){
